...
<svelte:head>
    <title>Leora Weddings | Ethiopia's #1 Luxury Wedding Platform</title>
</svelte:head>
    ...
    >
        <div class="space-y-8">
            ...
            <div class="flex flex-col gap-4 sm:flex-row">
                <Button href="/planner/budget" size="lg" class="flex items-center gap-2">
    ...
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div class="grid items-center gap-16 lg:grid-cols-2">
            <div>
                ...
                <Button href="/planner/budget" size="lg" class="mt-8">Try Budget Planner</Button>
            </div>
        ...
        <div class="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-6">
            {#each data.categories as cat (cat.id)}
                <a
                    ...
                    class="category-card glass-card cursor-pointer rounded-xl p-4 text-center border transition-all hover:border-leora-gold/50 hover:shadow-md"
                >
                    <div
                        class="mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-full bg-leora-gold/10"
                    >
                        <Building2 class="h-6 w-6 text-leora-gold" />
                    </div>
                    <h4 class="text-xs leading-tight font-medium text-leora-charcoal">{cat.name}</h4>
                    <p class="mt-1 text-xs text-gray-500">{cat.count} {cat.count === 1 ? 'vendor' : 'vendors'}</p>
                </a>
            {/each}
        </div>
    </div>
    ...
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid gap-8 md:grid-cols-3">
            {#each data.testimonials as t (t.id)}
                <div class="glass-card hover-lift rounded-2xl p-8 border">
                    <div class="mb-4 flex items-center gap-1">
                        {#each { length: 5 } as _}
                            <Star class="h-4 w-4 fill-leora-gold text-leora-gold" />
                        {/each}
                    </div>
                    <p class="mb-6 text-gray-600 italic">"{t.message}"</p>
                    <div class="flex items-center gap-3">
                        <img
                            src={t.avatar ?? 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=100&h=100&fit=crop'}
                            alt={t.name}
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-500">{t.position ?? 'Happy Couple'}</p>
                        </div>
                    </div>
                </div>
            {/each}
